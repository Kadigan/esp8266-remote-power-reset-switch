...
If you decide to use relays instead (to ensure maximum electrical separation) - that's also doable, but you'll want to use a driver like a ULN2003 or something similar (depending on the current, you may want to wire a pair of outputs per coil - it's as simple as shorting adjacent inputs together, and adjacent outputs together). ULN2003 comes equipped with the reverse diodes for spike dissipation, so it's a single solution. Not sure if a pull-down is necessary in this situation (or even for this board).
